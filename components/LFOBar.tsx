...
const LFOBar: React.FunctionComponent<Props> = ({lfoTypeID, lfoRateID, lfoAmountID, label, color}) => {
    ...
    return (
        <JuceComboBox parameterID={lfoTypeID}>
            ...
            <JuceSlider parameterID={lfoRateID}>
                {({value: lfoRateValue, onChange: lfoRateOnChange}) => {
                    const allowedNumerators = [1, 2, 3, 4]
                    const allowedDenominators = [1, 2, 4, 8, 16, 32]
                    const initialValues = functions.getFraction(lfoRateValue, allowedNumerators, allowedDenominators)
                    ...
                    useEffect(() => {
                        lfoRateOnChange(numerator / denominator)
